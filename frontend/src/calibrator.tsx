...
function calibrationFinished(): [Promise<undefined>, () => void] {
    ...
    const intervalID = setInterval(() => {
        axios.get('/isready', {cancelToken: cancelSource.token})
            .then((response) => {
                if (response.data) {
                    resolver();
                }
...
function CalibrationGoOutside(props: {onStart: () => void}) {
    return (
        <>
            ...
            <Card.Text>
                The outdoor air has a fairly consistent
                concentration of CO<sub>2</sub>. This well-known
                concentration will be used as a reference to
                calibrate the sensor. Click next once the
                device is outdoors.
            </Card.Text>
...
function Calibrating(props: {successful: boolean}) {
    ...
    return (
        <>
            ...
            <Card.Text>
                The sensor has started the calibration process. It takes
                several measurements for the sensor to calibrate itself, so
                this step may take up to a minute or two.
            </Card.Text>
            {button}
        </>
    );
...
class Wizard extends React.Component<WizardProps, WizardState> {
    ...
    async start() {
        ...
        let closeAfterCalibrationMs: number = 1500;
